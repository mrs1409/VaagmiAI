...
function initGlobeAnimation() {
  ...
  const cities = [
    { name: 'India',        lat:  38, lon:   0,  img: 'gallery-07.jpg' },
...
function initContactForm() {
  ...
  form.addEventListener('submit', (e) => {
    ...
    const btn = form.querySelector('button[type="submit"]') || form.querySelector('.contact__submit');
    const originalText = btn ? btn.textContent : '';
    if (btn) { btn.textContent = 'Sending...'; btn.disabled = true; }

    fetch('https://formspree.io/f/xeewjqbe', {
      method: 'POST',
